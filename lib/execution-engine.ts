...
async function executeLLMNode(
  ...
  try {
    let modelName = nodeData.model || 'gemini-2.0-flash'
...
async function executeCropNode(
  ...
  const response = await fetch(`${baseUrl}/api/process/crop`, {
    method: 'POST',
...
async function executeExtractFrameNode(
  ...
  const response = await fetch(`${baseUrl}/api/process/extract-frame`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      videoUrl,
  ...
  if (!response.ok) {
    const errorData = await response.json()
    throw new Error(errorData.error || 'Extract frame failed')
  }
